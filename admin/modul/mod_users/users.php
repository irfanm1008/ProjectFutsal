...
switch($_GET[act]){
    ...
    $warna="#fff";
     ...
     echo "<tr bgcolor=$warna>";
      ...
      echo " <td>$no</td>
             ...
             <td>$r[no_telp]</td>
         <td>$r[level]</td>
             <td><input type=button value='Edit' class='button' onclick=\"window.location.href='?module=user&act=edituser&id=$r[username]';\"></td></tr>";
      $no++;
    }
    echo "</table>";
    break;
  ...
  case "edituser":
    $edit=mysqli_query($db,"SELECT * FROM users WHERE username='$_GET[id]'");
